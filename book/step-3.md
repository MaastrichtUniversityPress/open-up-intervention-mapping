...
If you target a Change Objective with multiple applications or methods, copy its row, in such manner that you end up with an ABCD matrix where every row lists exactly one structural-causal chain.
...
```{tip} Examples
As you will see in the two examples, every ABCD matrix looks different. So, do not immediately think you did something wrong if it is not entirely similar to the examples.

- A list of student-produced ABCD matrices for different behaviors is available at <https://drive.google.com/drive/folders/1XnRnEjfNdWG7ifgcDR-OneWy2jkSJCYc?usp=sharing>
...
To achieve this, you need to *organize the applications* from column C of your ABCD matrix into components. Next, you decide on *the sequence*, the order in which the components will be presented, and decide on *the scope* of each component, for example whether there are topics that should not be addressed.
...
```{note} Guiding questions
- Generate ideas about possible themes, components, the scope and sequence of the program. The planning group develops an initial program plan that defines the program’s scope (extent and length), sequence (order of delivery), dosage (amount of the program to be delivered), and details for each interaction with target groups, including the required materials, staff, and resources.
```
...
![Logic Model of Change](figures/150---logic-model-of-change.png)
