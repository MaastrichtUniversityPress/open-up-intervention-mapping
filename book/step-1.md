...
Note: the `.epub` version of this workbook does not seem to be created properly yet. One problem is that these boxes do not have a color. If you happen to have expertise in this area and are willing to help out, it would be great if you would get in touch with us. The `.pdf` version seems ok.
...
In step 1, you do not only map out the problems and their causes, but you also map out your target population's landscape more generally. Specifically, it is important to identify in which setting the problem occurs, including aspects that may not play a role for the problem, understand the population, for example culture and existing communities, and other assets that you may be able to involve in the intervention later on. This is broadly referred to as the asset assessment. The result is a description of your target population and their surroundings.

This also includes the asset assessment, where you map the specific strengths of the population you want to intervene in. Assets are often mapped for four environment levels: the social environment, the information environment, the policy environment, and the physical environment. Keep in mind that assets are specific characteristics that are not generally true, but specific to your target population, for example exceptional infrastructure, closely knit community bonds, a specific social medium that is widely followed, or good adjustment of policies to local needs.

## Task: Stating program goals

Determine your program goals. You prioritize the potential goals on the basis of the needs assessment. This includes deciding what behaviors or environmental conditions are *most relevant* and *most likely to change*.
...
Note that you may want to change behaviors of environmental agents, too. However, in that case, often those behaviors are not what is important. Instead, the environmental condition that is caused or maintained by their behavior is important. That environmental condition, for example a certain norm, is what influences the problem or problems you want to address, so often you are better off specifying you want to change that environmental condition. In step 2, you will identify which behaviors each environmental agent needs to change in order to change the environmental condition.
...
Note that we separated increasing activity and decreasing weight. Here we might pick the parents as the target group, because we need them to bring the children to the extracurricular gymnastic classes, or the heads of schools in Amsterdam-Noord, who need to implement our intervention program at their school. The program goals should be feasible. Our examples might not be that feasible, but at least we focused at a specific age group of children in a specific area, Amsterdam-Noord, instead of "all children in the world".
```
